...
export const SESSION_COOKIE = "azeri_admin_session";
export const CASHIER_SESSION_COOKIE = "azeri_cashier_session";
const SESSION_TTL_SEC = 60 * 60 * 24 * 7;

const ADMIN_USER = process.env.ADMIN_USER ?? "adminazeri";
const ADMIN_PASS = process.env.ADMIN_PASS ?? "admin123";
const AUTH_SECRET = process.env.AUTH_SECRET ?? "dev-secret-change";
const CASHIER_USER = process.env.CASHIER_USER ?? "azerikassa";
...
export function verifyCashierCredentials(username: string, password: string) {
  return username === CASHIER_USER && password === CASHIER_PASS;
}
