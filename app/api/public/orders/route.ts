...
export async function POST(request: Request) {
  ...
  const create = db.transaction(() => {
    ...
    const orderResult = db
      .prepare(
        "INSERT INTO orders (customer_id, customer_address_id, total_amount, status, comment, created_at, updated_at) VALUES (?, ?, ?, 'paid', ?, ?, ?)"
      )
      .run(customerId, customerAddressId, totalAmount, comment, now, now);
