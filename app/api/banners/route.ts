...
export async function POST(request: Request) {
  ...
  const result = db
    .prepare(
      `INSERT INTO banners (title_ru, title_uz, image_url, link_url, sort_order, is_active, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(titleRu, titleUz, imageUrl, linkUrl, sortOrder, isActive, now, now);
